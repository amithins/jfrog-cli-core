...
func createMetadataTransferServiceManager(ctx context.Context, serverDetails *config.ServerDetails, httpClient *http.Client) (artifactory.ArtifactoryServicesManager, error) {
	return utils.CreateServiceManagerWithContextAndHttpClient(ctx, serverDetails, false, 0, metadataTransferRetries, metadataRetryWaitMilliSecs, time.Minute, httpClient)
}
